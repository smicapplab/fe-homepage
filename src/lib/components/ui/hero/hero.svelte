<script>
  ...
  <div
	class="hero min-h-[50vh] bg-base-200 relative"
	style={`background-image: url("${backgroundImage}");`}
  >
	<div class="absolute inset-0 bg-black opacity-50"></div>	
	<div
	  class="hero-content relative flex-col lg:flex-row lg:items-start lg:justify-start {imageLoaded ? 'content-visible' : ''}"
	>
	  <div class="w-full text-center text-white lg:w-5/6 lg:text-left">
		...
		{#if heroButton}
		  <div class="relative inline-flex group">
			<div
			  class="transition-all animate-tilt absolute -inset-px rounded-xl bg-gradient-to-r from-secondary via-[#FF44EC] to-primary opacity-70 blur-lg duration-1000 group-hover:-inset-1 group-hover:opacity-100 group-hover:duration-200"
			></div>
			{#if heroAnchorLink}
			  <button
