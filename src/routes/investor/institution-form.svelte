<script>
	...
	import { institutionInvestorContactSchema } from '$lib/schemas/contact';
	...
	const {
		...
		// @ts-ignore
		validators: zodClient(institutionInvestorContactSchema),
		dataType: 'json',
		onSubmit: () => {
			isLoading = true;
		},
...
<div class="w-full" id="reg-form">
	<form method="POST" class="grid gap-2 py-5" use:enhance action="?/institutionContact">
		...
		<Input label="Industry" name="industry" {formData} {errors} icon={Icons.building2} />
